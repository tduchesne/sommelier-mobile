...
import { 
  ...
  TextInput,
  Keyboard,
  useWindowDimensions // Pour la détection Tablette
} from 'react-native';
...
type Plat = {
  ...
  // Champs ajoutés pour le sprint 7
...
export default function MenuScreen() {
  // Hook pour la dimension
  const { width } = useWindowDimensions();
  const isTablet = width > 700; // Seuil pour passer en 2 colonnes
  ...
  // --- LOGIQUE INTELLIGENTE (Réintégrée) ---
  const filteredPlats = plats.filter(plat => {
    ...
    // 2. Filtre Allergènes avec Modificateurs
    ...
    const dishModifiables = plat.allergenesModifiables 
      ? plat.allergenesModifiables.toLowerCase().split(',').map(tag => tag.trim()) 
      : [];

    const isStrictlyForbidden = selectedAllergies.some(filter => {
      const filterLower = filter.toLowerCase();
      // Interdit si contient l'allergène ET qu'il n'est pas dans la liste des modifiables
  ...
  const renderItem = ({ item }: { item: Plat }) => {
    // Détection si modification requise
    const activeWarnings = selectedAllergies.filter(filter => {
        const dishAllergens = item.allergenes?.toLowerCase().split(',').map(t=>t.trim()) || [];
        return dishAllergens.includes(filter.toLowerCase());
    });
    ...
    return (
      <View style={[
          styles.card, 
          isTablet && styles.cardTablet, // Style grille
          isModifiedDisplay && styles.cardWarning // Style alerte
      ]}>
        ...
        {item.optionRemplacement && item.optionRemplacement !== "Aucune option de remplacement." ? (
           <View style={[styles.optionContainer, isModifiedDisplay && styles.optionContainerHighlight]}>
             ...
             <Text style={[styles.optionText, isModifiedDisplay && {color: '#d35400', fontWeight: 'bold'}]}>
                 {isModifiedDisplay ? item.optionRemplacement : "Option possible"}
             </Text>
           </View>
        ) : null}
      </View>
  ...
  return (
    <SafeAreaView style={styles.container}>
      {loading ? (
        ...
        <FlatList
          // La clé change quand le nombre de colonnes change -> Force le re-render propre
          key={numColumns} 
          data={filteredPlats}
...
const styles = StyleSheet.create({
  ...
  row: { justifyContent: 'space-between' }, // Espace entre les cartes en mode grille
  ...
  // Style spécifique Tablette
  cardTablet: {
    flex: 1, // Prend la largeur disponible
    marginHorizontal: 6, // Un peu d'espace latéral
    marginBottom: 16,
    maxWidth: '48%', // Sécurité pour 2 colonnes
  },
  cardWarning: {
